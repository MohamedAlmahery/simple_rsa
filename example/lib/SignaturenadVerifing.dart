...
class HomeState extends State<Home> {
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(
        title: new Text("Simple RSA Signature"),
      ),
      body: Padding(
            ...
            Padding(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: "Write the text to be signature",
                ),
              ),
            ),
            Row(
              ...
              children: <Widget>[
                RaisedButton(
                  child:
                  _stringEncoded.isEmpty ? Text("Signature") : Text("Verifing"),
                  color: Colors.yellow,
                  onPressed: () async {
                    String tmp = "";
                    try {
                      if (_stringEncoded.isEmpty) {
                        ...
                        if (text.isNotEmpty) {
                          final tmp = await signString(
                              myController.text, PRIVATE_KEY);
                          setState(() {
                            _stringEncoded = tmp;
                            myController.clear();
                          });
                        ...
                        tmp = (await verifyString(myController.text.toString(),_stringEncoded, PUBLIC_KEY)) as String;
                        setState(() {
                          _stringEncoded = "";
